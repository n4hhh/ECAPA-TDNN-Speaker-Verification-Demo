"""Execution-integrity checks for the frozen-handoff inference core."""
...
from src.inference import DEFAULT_CHECKPOINT_PATH, SpeakerVerifier, score_embeddings
from src.model import EMBEDDING_DIM, load_model

PROJECT_ROOT = Path(__file__).resolve().parents[1]
THESIS_CHECKPOINTS = (
    PROJECT_ROOT / "checkpoints" / "best_raw.pt",
    PROJECT_ROOT / "checkpoints" / "best_random.pt",
    PROJECT_ROOT / "checkpoints" / "best_adp.pt",
)
...
def _exercise_audio_pipeline(temp_dir: Path) -> Path:
    # Different tones per channel make this fixture exercise both resampling and
    # stereo-to-mono mixing instead of merely passing through an already valid clip.
    source_rate = 44_100
    duration_seconds = 4
    ...
    # A one-second ramp makes right-padding boundaries deterministic and easy to assert.
...
def _checkpoint_paths() -> tuple[Path, ...]:
    existing = tuple(path for path in THESIS_CHECKPOINTS if path.exists())
    return existing or (DEFAULT_CHECKPOINT_PATH,)


def _exercise_checkpoint(checkpoint_path: Path, audio_path: Path) -> tuple[tuple[int, ...], float]:
    model = load_model(checkpoint_path, device="cpu")
    assert not model.training

    # Low-amplitude synthetic audio checks tensor contracts and normalization only;
    # it intentionally makes no claim about speaker-discrimination quality.
    ...
    verifier = SpeakerVerifier(checkpoint_path, device="cpu")
    file_embedding = verifier.extract_embedding(audio_path)
    assert file_embedding.shape == (EMBEDDING_DIM,)
    assert torch.isfinite(file_embedding).all()
    return tuple(embedding.shape), embedding_norm


def main() -> None:
    torch.manual_seed(0)

    results: list[tuple[Path, tuple[int, ...], float]] = []
    with tempfile.TemporaryDirectory(prefix="speaker_verification_smoke_") as temp_name:
        ...
        for checkpoint_path in _checkpoint_paths():
            shape, norm = _exercise_checkpoint(checkpoint_path, stereo_path)
            results.append((checkpoint_path, shape, norm))
    ...
    for checkpoint_path, shape, norm in results:
        print(f"{checkpoint_path.name}: embedding shape={shape} norm={norm:.8f}")
    print(
